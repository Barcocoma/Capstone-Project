...
try {
    ...
    // Insert into customers table using prepared statement - balanced fields
    $sql = "INSERT INTO customers (first_name, last_name, middle_name, email, phone, alternate_phone, date_of_birth, gender, civil_status, street_address, city, province, emergency_contact_name, emergency_contact_phone, occupation, employer, monthly_income, last_payment_date, status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $result = $stmt->execute([
        ...
        $emergency_contact_phone, $occupation, $employer, $monthly_income, $last_payment_date, $status
    ]);
    ...
    if ($result) {
        ...
        $activity_stmt->execute([
            'Created',
            'Customer',
            "New customer '$first_name $last_name' added",
            $actorId,
            $_SERVER['REMOTE_ADDR'] ?? '',
